...
//}
//
//func (b *TestBroker) run(w http.ResponseWriter) {
...
//			service.AddUserOnlineService(client.user)
//		case client := <-b.unregister:
//			if _, ok := b.clients[client]; ok {
//				//delete in database when client off
//				service.DeleteUserOnlineService(client.user.SocketID)
...
//	fmt.Println(user)
//
//	client := &Client{user: user, broker: broker, conn: conn, send: make(chan []byte, 256)}
